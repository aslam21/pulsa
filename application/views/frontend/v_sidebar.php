...
<div class="widget-sidebar">
  <h5 class="sidebar-title">event Terbaru</h5>
  <div class="sidebar-content">
    <ul class="list-sidebar">
      <?php 
      $event = $this->db->query("SELECT * FROM event,pengguna,kategori WHERE event_status='publish' AND event_author=pengguna_id AND event_kategori=kategori_id ORDER BY event_id DESC LIMIT 3")->result();
      foreach($event as $a){
        ?>
        <li>
          <a href="<?php echo base_url().$a->event_slug; ?>"><?php echo $a->event_judul; ?></a>
        </li>
        <?php
      }
      ?>
